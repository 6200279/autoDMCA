...
resource "aws_iam_policy" "eks_application_policy" {
  name        = "${local.name_prefix}-eks-application-policy"
  description = "IAM policy for EKS applications"
...
resource "aws_iam_role_policy_attachment" "eks_application_policy" {
  policy_arn = aws_iam_policy.eks_application_policy.arn
  role       = aws_iam_role.eks_node_group.name
}
...
resource "aws_iam_policy" "cluster_autoscaler" {
  count       = var.enable_cluster_autoscaler ? 1 : 0
  name        = "${local.name_prefix}-cluster-autoscaler"
  description = "EKS cluster-autoscaler policy for cluster ${aws_eks_cluster.main.name}"
